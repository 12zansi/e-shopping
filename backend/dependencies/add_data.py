...
from backend.schemas.product_details import ProductDetail, ProductImage
...
class AddData(AddDataInterface):
    ...
    def add_category(self, category: CategoryDetail):
        new_category = TBCategory(category_name = category.category_name, brand_name = category.brand_name,
                                  b_id=category.b_id)
    ...
    def add_produck_images(self, images: list[UploadFile] = File(...), product_id: int = Form(...), color: str = Form(...), total_stock: int = Form(...),  db: Session = Depends(start_session)):
        add = []
        for image in images:
            ...
            new_images = TBProductStock(
                product_id=data.product_id, 
                image_name=data.image,
                color=data.color, 
                total_stock=data.total_stock)
                
            AddData._add_in_database(new_images, db)
            add.append(new_images)
            print(data)
    ...
    def add_cart(self, file1: UploadFile, product_name: str = Form(...), price: int = Form(...), quantity: int = Form(...), r_id: int = Form(...)):
        ...
        cart = TBCart(product_name=mapping_cart.product_name,
                      ...
                      total_price=total_price, c_image=image_name,
                      r_id=mapping_cart.r_id)

        return AddData._add_in_database(self,cart)

    def add_in_place_order(self, in_place_order: PlaceOrder):
        place_order = TBPlaceOrder(username=in_place_order.username, email=in_place_order.email, products=in_place_order.products,
                                   total_price=in_place_order.total_price,  address=in_place_order.address, delivery_type=in_place_order.delivery_type, r_id=in_place_order.r_id)
